...
use fig::compress_data;
use tauri::api::path::config_dir;

#[tauri::command]
async fn save_data(data: String) -> Result<(), String> {
  let path = Path::new(&config_dir().ok_or("Config dir is not supported by your platform.")?).join("fig-soundcloud").join("data.dat");
...
async fn load_data() -> Result<String, String> {
  ...
  let data = fig::decompress_data(compressed).map_err(|e| e.to_string())?;
